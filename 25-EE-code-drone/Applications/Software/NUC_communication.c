...
void STM32_to_MINIPC()
{
    ...
    toMINIPC.To_minipc_data.curr_omega = cos(IMU_data.AHRS.pitch)*IMU_data.gyro[2]-sin(IMU_data.AHRS.pitch)*IMU_data.gyro[0];//¶à½Ç¶ÈÈÚºÏ;
...
void MINIPC_to_STM32()
{
    ...
    if (fabs(fromMINIPC.from_minipc_data.shoot_yaw - IMU_data.AHRS.yaw) > PI / 2.0f)
    {
...
void Auto_control()
{
    if(Global.Auto.input.Auto_control_online==1)
		{
			Gimbal_set_yaw_angle(Global.Auto.input.shoot_yaw);
			Gimbal_set_pitch_angle(Global.Auto.input.shoot_pitch);
